...
do_install() {
    ...
    for p in libgstapp.so libgstvideoconvertscale.so libgstvideoconvert.so libgstrtsp.so libgstrtp.so libgstrtpmanager.so libgstvideoparsersbad.so libgstisomp4.so libgstplayback.so libgstmatroska.so ; do
